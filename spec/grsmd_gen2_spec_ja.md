...
| OS-07 | コンテンツによるファイルタイプ検出（マジックバイト） | 拡張子ベースのルーティングで十分 |
...
| OS-17 | バイナリファイルのコンテンツ検出（マジックバイト判定） | 拡張子ベースの判定で十分。拡張子を擬態したファイルはそのまま表示を試みる（特別な対処不要）。 |
...
| INV-17 | イベント駆動 | `.md` ファイルがドロップされた場合、システムはそれをMarkdownプレビューにルーティングすること。EXT\_TO\_HLJS に登録済みの拡張子を持つファイルがドロップされた場合、システムはそれをコードビューにルーティングすること。 |
| INV-18 | イベント駆動 | EXT\_TO\_HLJS に未登録かつ `.md` でもない拡張子のファイルがドロップされた場合、システムはアラートでエラーメッセージを表示しファイルを処理しないこと。拡張子を擬態したファイル（登録済み拡張子だが中身がバイナリ等）に対しては特別な対処を行わず、そのまま表示を試みること。 |
...
任意の状態から `.md` ドロップまたはペーストで `markdown` へ、サポート対象の非`.md` ファイルドロップで `code` へ遷移可能。サポート外の拡張子はアラートで拒否し、状態遷移は発生しない (INV-18)。
...
Feature: Quick Code View
  ...
  Scenario: EXT_TO_HLJSに未登録の拡張子のファイルをドロップ
    Given アプリケーションが任意の状態である
    When ユーザーがEXT_TO_HLJSに未登録の拡張子 ".xyz" のファイルをドロップする
    Then アラートに "Unsupported file type." が表示される
    And ファイルは処理されない (INV-18)
...
Feature: ファイルサイズ検証
  ...
  Scenario: サポート外の拡張子のファイルをドロップ
    Given 任意のアプリケーション状態
    When ユーザーがEXT_TO_HLJSに未登録かつ.mdでもない拡張子のファイルをドロップする
    Then アラートに "Unsupported file type." が表示される
    And ファイルは処理されない
  ...
  Scenario: 拡張子を擬態したファイルをドロップ
    Given 任意のアプリケーション状態
    When ユーザーがサポート対象の拡張子だが中身がバイナリのファイルをドロップする
...
以下のすべての言語IDは highlight.js 11.11.1 に対して検証済み。このテーブルは受け入れ可能なファイル拡張子のホワイトリストとしても機能する: `.md` でもなく、このテーブルにも登録されていない拡張子のファイルはドロップ時にアラートで拒否される (INV-18)。テーブル内の拡張子で `hljs.highlightAuto()` にフォールバックするケースは、言語値が `null` に解決された場合のみ。自動検出も失敗した場合、コンテンツはエラーなしにプレーンテキストとして表示される。
...
flowchart TD
    ...
    C -->|"no"| E{"extension lowercased"}
    E -->|".md"| F[load text into editor textarea]
    E -->|"in EXT_TO_HLJS"| G[await handlePromise then SourceFileRenderer.render]
    E -->|"not in EXT_TO_HLJS and not .md"| H([alert unsupportedType and abort])
    F -->|"auto render"| I(["_viewState = markdown"])
    G -->|"highlight and line numbers"| J(["_viewState = code"])
```
...
document.addEventListener("drop", async (e) => {
  ...
  if (ext === ".md") {
    ...
    orchestrator.loadMarkdown(text);
    ...
    alert(CONFIG.fileDrop.messages.unsupportedType);
  }
});
```
