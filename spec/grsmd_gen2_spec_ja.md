...
graph TB
    subgraph Browser
        subgraph GRSMD_SPA["GRSMD Single Page Application"]
            ...
            MD["MarkdownRenderer<br/>(marked.js + hljs)"]:::adapter
            MERM["MermaidRenderer<br/>(mermaid.js)"]:::adapter
            PUML["PlantUMLRenderer<br/>(plantuml-encoder)"]:::adapter
            SRC["SourceFileRenderer<br/>(hljs Code View)"]:::adapter
            SCROLL["SmoothScrollEngine<br/>(RAF Physics)"]:::adapter
            SMGR["ScrollManager<br/>(Position Save and Restore)"]:::adapter
...
**CA レイヤー凡例:** Entity(橙) = ドメインデータ / Use Case(金) = ビジネスロジック調整 / Adapter(緑) = 外部ライブラリ・ブラウザAPI適合 / Framework(青) = UIイベント・デバイス入力
...
classDiagram
    ...
    ApplicationState:::entity
    CodeViewMeta:::entity
    RendererOrchestrator:::usecase
    MarkdownRenderer:::adapter
    MermaidRenderer:::adapter
    PlantUMLRenderer:::adapter
    SourceFileRenderer:::adapter
    ScrollManager:::adapter
    SmoothScrollEngine:::adapter
    ModalController:::adapter
    DiagramRenderer:::adapter
    UIController:::framework
    DiagramZoomController:::framework
    DiagramPanController:::framework
    DiagramCopyController:::framework
```
